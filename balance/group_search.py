...
df['search_time'] = np.nan

for index, row in df.iterrows():
	rv = None
	if index >= 11692 and row['Tree'] == 1:
		name = row['name']
		time = row['time']
		...
		for hour in range(8, 24):
			for minute in range(0, 6):
				file_name = folder_1 + (("0" + str(hour)) if hour <= 9 else str(hour))+ ":" + str(minute) + "0.json"
		...
		if rv == None:
			...
			for hour in range(0, 7):
				for minute in range(0, 6):
					file_name = folder_2 + (("0" + str(hour)) if hour <= 9 else str(hour))+ ":" + str(minute) + "0.json"
					time_list.append(file_name)
...
rv_df = df[df['index'] >= 11692][['time', 'name', 'price_3', 'price', 'price_increase', 'search_time']]

rv_df.to_json('tree_search_time.json', orient = 'values')
